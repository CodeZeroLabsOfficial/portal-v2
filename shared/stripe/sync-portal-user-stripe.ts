...
export async function syncPortalUserFromCrmCustomerDoc(db: Firestore, customerId: string): Promise<void> {
  ...
  try {
    ...
    if (!email) {
      console.error("sync_portal_user_missing_email", { customerId: id, portalUserId });
      return;
    }
    ...
    console.error("sync_portal_user_from_crm_failed", {
      customerId: id,
...
export async function syncStripeCustomerIdFromCrmCustomerDoc(
  db: Firestore,
  customerId: string,
): Promise<void> {
